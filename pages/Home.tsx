...
const PARALLAX_PRODUCTS = [...MOCK_TRACKS, ...MOCK_TRACKS].slice(0, 15).map(t => ({
  title: t.title,
  link: `/track/${t.id}`,
...
const MOODS = [
  { name: 'Chill', color: 'from-blue-500 to-cyan-400', icon: Wind },
  { name: 'Energy', color: 'from-red-500 to-orange-400', icon: Zap },
  { name: 'Focus', color: 'from-purple-500 to-indigo-400', icon: Brain },
  { name: 'Party', color: 'from-yellow-400 to-pink-500', icon: Sparkles },
];

const FEATURED: Track = MOCK_TRACKS[2]; // Neon Nights

const TRENDING: Track[] = MOCK_TRACKS.slice(0, 5);

const NEW_RELEASES: Track[] = MOCK_TRACKS.slice(5, 9);

const SPOTLIGHT_TRACKS: Track[] = MOCK_TRACKS.slice(8, 12);
...
const NewReleaseCard: React.FC<{ track: Track; index: number }> = ({ track }) => {
    ...
    return (
      <div 
          className="group relative aspect-video w-[350px] md:w-[450px] rounded-xl overflow-hidden cursor-pointer flex-shrink-0"
  ...
  return (
    <PageTransition>
      <div className="min-h-screen pb-32">
        ...
        <div className="relative z-10 space-y-24 pb-20 mt-12">
          ...
          {/* Made For You Section - Infinite Scroll */}
          <section className="max-w-[100vw] overflow-hidden">
            <div className="px-8 max-w-[1920px] mx-auto">
              <h3 className="text-3xl font-bold mb-8 text-white">Made For {user ? user.name : 'You'}</h3>
            </div>
            <div className="relative w-full">
               <InfiniteMovingCards
